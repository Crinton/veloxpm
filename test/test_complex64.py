...
def generate_matrix(size: int) -> np.ndarray:
    ...
    matrix = rng.standard_normal((size, size)).astype(np.float32)
    matrix = -0.5 * (matrix + matrix.T)
    norm1 = np.linalg.norm(matrix, ord=1)
    if norm1 > 0:
        matrix *= np.float32(0.25 / norm1)
    return np.ascontiguousarray(matrix)


def main() -> None:
    print("complex64: exp(iH), H is real")
    ...
    for size in SIZES:
        ...
        t0 = time.perf_counter()
        reference = expm((1j * matrix).astype(np.complex64))
